...
use Rosio\EncryptedCookie\CryptoSystem\AES_SHA256;

class AES_SHA256Test extends \PHPUnit_Framework_TestCase
{
	protected $cryptoSystem;

	function setUp ()
	{
		$this->cryptoSystem = new AES_SHA256($this->getRandom(32), "SomeHMAC");
	}
	...
	{
		$this->assertTrue(AES_SHA256::ctComp('', ''));
		$this->assertTrue(AES_SHA256::ctComp('data', 'data'));

		$this->assertFalse(AES_SHA256::ctComp('', 'data'));
		$this->assertFalse(AES_SHA256::ctComp('data', ''));

		$this->assertFalse(AES_SHA256::ctComp('data', 'as'));
		$this->assertFalse(AES_SHA256::ctComp('as', 'data'));

		$this->assertFalse(AES_SHA256::ctComp(chr(0), chr(1)));
		$this->assertTrue(AES_SHA256::ctComp(chr(1), chr(1)));
	}
